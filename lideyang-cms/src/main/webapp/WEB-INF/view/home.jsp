		...
		<div class="row">
			...
			<div class="col-md-7 split min_h_500">
				...
				<c:choose>
					<c:when test="${not empty article.picture}">
						<div class="media">
						  <div class="media-left">
						    <a href="#">
						      <img class="media-object" src="${article.picture}" width="120" style="${article.style}" alt="${article.title }">
						    </a>
						  </div>
					...
					<c:otherwise>
						  <div>
						    <h3><a class="article_title" href="/blog/${article.id}" style="${article.style}">${article.title }</a></h3>
						    <p>${article.summary}</p>
						    <p class="blog_item_footer">
						    	<span class="glyphicon glyphicon-user" title="作者"></span>作者：${article.author.nickname}&nbsp;&nbsp;&nbsp;
						    	<span class="glyphicon glyphicon-time" title="发布时间"></span>发布：&nbsp; <fmt:formatDate value="${article.updated}" pattern="yyyy-MM-dd HH:mm:ss"/>&nbsp;&nbsp;&nbsp;&nbsp; 
			...
			<div class="col-md-3">
				...
				<%-- <div class="card">
					...
					<div class="card-body">
							<ol>
								<c:forEach items="${lastArticlesPhoto}" var="article">
								<li class="text-truncate"><img class="media-object" src="${article.picture}" width="120" alt="${article.title }">
									<a href="/article/${article.id}" style="${article.style}">${article.title}</a>
								</li>
								</c:forEach>
							</ol>
					</div>v
